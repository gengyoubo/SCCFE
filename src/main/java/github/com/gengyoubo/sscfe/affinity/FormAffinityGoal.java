...
/** Makes a vanilla species follow players whose active form matches that species. */
public final class FormAffinityGoal extends Goal {
    ...
    private boolean isValidTarget(Player player) {
        return player != null && player.isAlive() && !player.isSpectator() && matchesSpecies(player, mob);
    }

    private float movementSpeed() {
        if (mob instanceof Axolotl) return 0.75F;
        if (mob instanceof Bat || mob instanceof Allay) return 0.70F;
        return 0.75F;
    }

    public static boolean supports(Mob mob) {
        return mob instanceof Axolotl || mob instanceof Bat || mob instanceof Cat || mob instanceof Fox
                || mob instanceof Ocelot || mob instanceof Wolf || mob instanceof Allay || mob instanceof Spider;
    }
    ...
    private FollowPoint followPoint(Player player) {
        ...
        return new FollowPoint(player.getX() - directionX * followDistance, player.getY(),
