...
public final class AxolotlBreedingService {
    ...
    public static void playerTick(LivingEvent.LivingTickEvent event) {
        if (!(event.getEntity() instanceof Player player) || player.level().isClientSide || !isAxolotlForm(player)) {
            return;
        }

        long now = player.level().getGameTime();
        for (Axolotl axolotl : player.level().getEntitiesOfClass(Axolotl.class,
                        player.getBoundingBox().inflate(SEARCH_RADIUS), candidate -> candidate.isAlive()
                                && !candidate.isBaby() && candidate.canFallInLove() && !candidate.isInLove())
    ...
    public static void breedingSucceeded(BabyEntitySpawnEvent event) {
        ...
        if (!(event.getChild() instanceof Axolotl) || player == null || !isAxolotlForm(player)) {
            return;
        }
    ...
    private static void markCooldown(Mob parent, Player player, long cooldownUntil) {
        if (!(parent instanceof Axolotl axolotl)) return;
        UUID axolotlId = axolotl.getUUID();
        UUID attemptingPlayer = ATTEMPTING_PLAYER.remove(axolotlId);
        if (player.getUUID().equals(attemptingPlayer) || player.equals(axolotl.getLoveCause())) {
